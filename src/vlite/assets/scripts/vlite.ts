...
class Vlitejs {
	...
	onKeydown(e: KeyboardEvent) {
		// Prevent Keyboard Events for specific HTML tags
		const disallowedTargetTags = ['input', 'textarea'];
		if (disallowedTargetTags.indexOf((e.target as Element).tagName.toLowerCase()) > -1) {
			return;
		}
